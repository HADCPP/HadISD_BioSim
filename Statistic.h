...
//
enum TStat{ LOWEST, MEAN, SUM, SUM², STD_DEV, STD_DEV_OVER_POP, STD_ERR, COEF_VAR, VARIANCE, HIGHEST, TSS, QUADRATIC_MEAN, RANGE, NB_VALUE, NB_STAT_TYPE };
...
enum TStatEx{ MAD = NB_STAT_TYPE, SKEWNESS, KURTOSIS, MEDIAN, Q² = MEDIAN, MODE, Qᴸ, Q¹, Q³, Qᴴ, INTER_Q, NB_STAT_TYPE_EX,MED=QUADRATIC_MEAN};
//MEAN_X	: mean of x
//MEAN_Y	: mean of y
...
enum TStatXYEx{INTERCEPT_THEIL_SEN=NB_STATXY_TYPE, SLOPE_THEIL_SEN, NEGATIVE_LOG_LIKELIHOOD, NB_STATXY_TYPE_EX};
...
public:
	...
	CStatistic();
	CStatistic(const CStatistic& in){operator=(in);}
	CStatistic(double value){operator=(value);}
	...
	CStatistic& Add(double value){ return operator+=(value); }
	CStatistic& insert(double value){ return operator+=(value); }
	CStatistic& operator+=(double value);
	CStatistic& operator+=(const CStatistic& statistic);
	bool operator==(const CStatistic& in)const;
	bool operator!=(const CStatistic& in)const{ return !operator==(in); }

	CStatistic operator+(const CStatistic& in){ return CStatistic(*this)+=in;}
	CStatistic& operator=(const CStatistic& in);
	CStatistic& operator=(double value){Reset();return operator+=(value);}
	operator bool()const{ return IsInit(); }
	operator double()const{ return operator[](WBSF::MEAN); }

	bool empty()const{ return m_nbValues==0;}
	bool IsInit()const{ return !empty();}
	
	static double GetVMiss();
	static void SetVMiss(double vmiss);

	static const char* GetName(size_t s){ _ASSERTE(s<NB_STATXY_TYPE_EX); return NAME[s]; }
...
public:
	...
	CStatisticEx& Add(double value){ return operator+=(value); }
	CStatisticEx& insert(double value){ return operator+=(value); }
	CStatisticEx& operator+=(double value);
	CStatisticEx& operator+=(const CStatisticEx& statistic);
	
	//static const char* GetName(int i){ _ASSERTE(i >= 0 && i<NB_STAT_TYPE_EX); return i<NB_STAT_TYPE ? CStatistic::GetName(i) : NAME[i - NB_STAT_TYPE]; }

	//const std::vector<double>& GetValues(){ return m_values; }
	const std::vector<double>& operator()(void)const { return m_values; }
	size_t size()const{ return m_values.size(); }
	double operator()(size_t i)const{ return m_values[i];}
	double get_sorted(size_t i)const{ sort();  return m_sorted[i]; }
...
//*************************************************************
//CStatisticW: keep in memory for weathed statistic
class CStatisticXW
{
public:

	CStatisticXW();
	CStatisticXW(const CStatisticXW& in){ operator=(in); }
	CStatisticXW(double value, double weight){ Add(value, weight); }
	...
	CStatisticXW& insert(double value, double weight){ return Add(value, weight); };
	CStatisticXW& operator+=(const CStatisticXW& statistic);
	bool operator==(const CStatisticXW& in)const;
	bool operator!=(const CStatisticXW& in)const{ return !operator==(in); }

	CStatisticXW operator+(const CStatisticXW& in){ return CStatisticXW(*this) += in; }
	CStatisticXW& operator=(const CStatisticXW& in);

	bool empty()const{ return m_x.empty(); }
	bool IsInit()const{ return !empty(); }

	const CStatisticEx& X()const { return m_x; }
	const CStatisticEx& W()const { return m_w; }
	double X(size_t i)const { return m_x[i]; }
	double W(size_t i)const { return m_w[i]; }

	const std::vector<double>& x()const { return m_x(); }
	const std::vector<double>& w()const { return m_w(); }
	double x(size_t i)const { return m_x(i); }
	double w(size_t i)const { return m_w(i); }
...
public:
	...
	CStatisticXY();
	CStatisticXY(const CStatisticXY& in){operator=(in);}
	~CStatisticXY(){};
	void Reset();
	void clear(){ Reset(); }
	...
	CStatisticXY& insert(double x, double y){ return Add(x, y); }
	CStatisticXY& operator+=(const CStatisticXY& statistic);

	const CStatistic& GetX()const{ return m_x; }
	const CStatistic& GetY()const{ return m_y; }
	const CStatistic& X()const{ return m_x; }
	const CStatistic& Y()const{ return m_y; }
	double X(size_t t)const{ return m_x[t]; }
	double Y(size_t t)const{ return m_y[t]; }
...
protected:
	...
	double m_hightest;	//higest error
};
...
public:
	...
	CStatisticXYEx();
	CStatisticXYEx(const CStatisticXYEx& in){operator=(in);	}//need copy constructor for m_CS
	~CStatisticXYEx(){}; 
	...
	CStatisticXYEx& insert(double x, double y){ return Add(x, y); }
	CStatisticXYEx& operator+=(const CStatisticXYEx& statistic);
	
	const std::vector<double>& x()const {return m_xValues;}
	const std::vector<double>& y()const {return m_yValues;}
...
public:
	...
	CStatisticXYW();
	CStatisticXYW(const CStatisticXYW& in){ operator=(in); }
	CStatisticXYW(double x, double y, double w){ insert(x, y, w); }
	...
	const CStatisticXW& X()const{ return m_xw; }
	const CStatisticXW& Y()const{ return m_yw; }
	const CStatisticEx& W()const{ return m_xw.W(); }
	double X(size_t t)const{ return m_xw.X(t); }
	double Y(size_t t)const{ return m_yw.X(t); }
	double W(size_t t)const{ return m_xw.W(t); }


	const std::vector<double>& x()const { return m_xw.x(); }
	const std::vector<double>& y()const { return m_yw.x(); }
	const std::vector<double>& w()const { return m_xw.w(); }

	double x(size_t i)const { return m_xw.x(i); }
	double y(size_t i)const { return m_yw.x(i); }
	double w(size_t i)const { return m_xw.w(i); }
...
public:
	...
	bool operator ==(const CXY& in)const{ return m_x==in.m_x && m_y == in.y; }
	bool operator !=(const CXY& in)const{ return !operator==(in); }
	bool operator >(const CXY& in)const{ return m_x > in.m_x; }
	bool operator >=(const CXY& in)const{ return m_x >= in.m_x; }
	bool operator <(const CXY& in)const{ return m_x < in.m_x; }
	bool operator <=(const CXY& in)const{ return m_x <= in.m_x; }
};
...
public:
	...
	int GetSize()const{ return (int)m_stat.size(); }
	size_t size()const{ return m_stat.size(); }
	const CXYStat& operator [](int i)const{ _ASSERTE(i>=0 && i<(int)m_stat.size()); return m_stat[i]; }
...
public:
	...
	double operator()(double x){return m_a0 + x*m_a1; }
