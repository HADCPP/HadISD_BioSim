...
namespace WBSF
{
	...
	double CStatistic::operator[](size_t type)const
	{
		...
		if (m_nbValues > 0 || type == NB_VALUE)
		{
			...
			case STD_DEV:value = sqrt(std::max(0.0, (m_sum² - (m_sum*m_sum) / m_nbValues)) / (nbVal)); break;
			case STD_DEV_OVER_POP:value = sqrt(std::max(0.0, (m_sum² - (m_sum*m_sum) / m_nbValues)) / (m_nbValues)); break;
			case STD_ERR:value = (me[STD_DEV] / sqrt((double)m_nbValues)); break;
			case TSS: value = std::max(0.0, m_sum² - (m_sum*m_sum) / m_nbValues); break;
			case COEF_VAR:
				if (!(m_sum² > 0 && m_sum == 0))
				{
			...
			case VARIANCE:value = std::max(0.0, (m_sum² - (m_sum*m_sum) / m_nbValues) / (nbVal)); break;
	...
	void CStatisticEx::sort()const
	{
		...
		_ASSERTE(m_sorted.size() == m_values.size());
	}
	...
	{
		_ASSERTE(m_x[NB_VALUE] == m_w[NB_VALUE]);
		_ASSERTE(type >= 0 && type < NB_STAT_TYPE);
		...
		if (nbValues > 0 || type == NB_VALUE)
		{
			...
			case VARIANCE: value = ((Square(m_w[SUM]) - m_w[SUM²]) != 0) ? (m_wx² * m_w[SUM] - Square(m_wx)) / (Square(m_w[SUM]) - m_w[SUM²]) : STAT_VMISS; break;
			...
			default: _ASSERTE(false);//not supported by this class
			}
		}
	...
	double LogFactorial(double x) { return 0; }
	...
	{
		_ASSERTE(m_xw[NB_VALUE] == m_yw[NB_VALUE]);
		_ASSERTE(type >= 0 && type < NB_STATXY_TYPE);
